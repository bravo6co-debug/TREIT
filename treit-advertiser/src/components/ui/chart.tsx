...
const ChartStyle = ({ id, config }: { id: string; config: ChartConfig }) => {
  ...
  const sanitizeKey = (key: string): string => {
    ...
    // CSS 변수명에서 허용되지 않는 문자들 제거
    return key.replace(/[^a-zA-Z0-9-_]/g, '');
  };

  const safeCSS = Object.entries(THEMES)
    .map(([theme, prefix]) => {
      const themePrefix = sanitizeCSS(prefix);
      const chartId = sanitizeCSS(id);
      ...
      return chartId && colorRules ? `${themePrefix} [data-chart="${chartId}"] {\n${colorRules}\n}` : '';
    })
    .filter(Boolean)
    .join('\n');
